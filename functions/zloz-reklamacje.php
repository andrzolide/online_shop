...
$zapytanie= "";
  ...
  if($id_zam!=""&&$opis!=""&&$id_prod_bez_nr!=""){
  	if($nr_ser==""){
	    $zapytanie = "INSERT INTO reklamacje (id, id_zamowienia, id_produktu_bez_seryjnego, powod)
	      VALUES ('NULL', '$id_zam', '$id_prod_bez_nr', '$opis')";
  	}else{
  		$zapytanie = "INSERT INTO reklamacje (id, id_zamowienia,  nr_seryjny, powod)
  ...
  if ($polaczenie->query($zapytanie) === TRUE) {
    ...
    $message="info=Pomyślnie złożono reklamację";
  } else {
    $message="blad=". "Błąd przy składaniu reklamacji: " . $zapytanie . "<br>" . $polaczenie->error;
