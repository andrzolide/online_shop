...
$zapytanie= "";
  ...
  if($id_zam!=""&&$opis!=""&&$id_prod_bez_nr!=""&&$status=="zrealizowane"){
  	if($nr_ser==""){
	    $zapytanie = "INSERT INTO `reklamacje` (`id`, `id_zamowienia`, `id_produktu_bez_seryjnego`, `powod`, `status`)
	      VALUES (NULL, $id_zam, $id_prod_bez_nr, '$opis', 'zgloszono')";
  	}else{
  		$zapytanie = "INSERT INTO `reklamacje` (`id`, `id_zamowienia`,  `nr_seryjny`, `powod`, `status`)
  ...
  if ($polaczenie->query($zapytanie) === TRUE) {
    ...
    $message="blad=Nie udało się zlozyc reklamacji";
